...
causalTree_combined = Var_test_algorithm(effect = E, Method = 'Combined', item = 1000,
                                     Alpha = Alpha, Beta = Beta, Gama = Gama,
                                     inter_coef = 0.2875, BuckN = n, BuckM = m)
bias_causalTree_combined = causalTree_combined[1]
std_causalTree_combined = causalTree_combined[2]
RMSE_causalTree_combined = causalTree_combined[3]


causalTree_Truncated = Var_test_algorithm(effect = E, Method = 'Truncated', 
                                      Truncated.alpha = 0.1, item = 1000,
                                      Alpha = Alpha, Beta = Beta, Gama = Gama,
                                      inter_coef = 0.2875, BuckN = n, BuckM = m)
bias_causalTree_Truncated = causalTree_Truncated[1]
std_causalTree_Truncated = causalTree_Truncated[2]
RMSE_causalTree_Truncated = causalTree_Truncated[3]

causalTree_Overlap = Var_test_algorithm(effect = E, Method = 'Overlap', item = 1000,
                                    Alpha = Alpha, Beta = Beta, Gama = Gama,
                                    inter_coef = 0.2875, BuckN = n, BuckM = m)
bias_causalTree_Overlap = causalTree_Overlap[1]
...
write.csv(ans, "ATE&CT_sq_ans_3st.csv")
ans
